...
define('API_RATE_LIMIT', 900);        // Requests per minute
define('SSL_VERIFY', false);          // SSL verification (false for dev, true for prod)
define('MAX_EXECUTION_TIME', 360);    // Max runtime in seconds (6 minutes)
define('MEMORY_LIMIT', '6048M');      // Memory limit for large datasets

// Initial record ID for first sync
$STARTING_RECORD_ID = 0; // Can be changed to start from a specific record_id

// Forms to EXCLUDE from sync (add forms that should NOT be synced)
$excluded_forms = [
    "survey_admin",                  // Example form to exclude
...
define('REQUIRED_FIELDS', ['record_id']); // Always include record_id
...
function sync_local_to_remote($rateLimiter) {
    ...
    // Export new record IDs from local (always include record_id)
    $export_params = [
    ...
    foreach (array_chunk($records, BATCH_SIZE) as $batch) {
        ...
        // Prepare batch data (ensuring record_id is included)
        $import_data = array_map(function($r) { 
...
function get_all_forms($rateLimiter) {
    global $config, $excluded_forms;
    ...
    $metadata = redcap_api_call($config['api']['remote_url'], $params, $rateLimiter);
    
    // Get unique form names and exclude the ones we don't want
    $all_forms = array_unique(array_column($metadata, 'form_name'));
    $forms_to_sync = array_diff($all_forms, $excluded_forms);
    
    return array_values($forms_to_sync); // Return as indexed array
}
...
function sync_remote_to_local($rateLimiter) {
    ...
    // Get all forms except excluded ones
    $forms = get_all_forms($rateLimiter);
    
    if (empty($forms)) {
        log_message("No forms to sync (all may be excluded)");
        return 0;
    }
    
    log_message("Syncing forms: " . implode(", ", $forms));
    ...
    // Always include record_id in fields
    $fields_params = [];
    $fields_params["fields[0]"] = 'record_id';
    ...
    foreach (array_chunk($records, BATCH_SIZE) as $batch) {
        ...
        // Ensure record_id is preserved in the import
        $import_params = [
            'token' => $config['tokens']['local_token'],
            'content' => 'record',
            'format' => 'json',
